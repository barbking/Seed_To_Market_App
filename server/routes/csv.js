              ...
              if (err) {
                ...
                if (req.params.fromDate === undefined && req.params.toDate === undefined) {
                  var jsonQueryAllSeeds = 'SELECT * FROM seeds WHERE user_id= $1',
                    [req.user.user_id];
                  db.query(jsonQueryAllSeeds, function(queryError, result) {
                    done();
                    if (queryError) {
                      res.sendStatus(500);
                    } else {
                      ...
                      var jsonQuerySeeds = 'SELECT * FROM seeds WHERE purchase_date >=$1 and purchase_date <=$2 and user_id= $3',
                        [req.params.fromDate, req.params.toDate, req.user.user_id];
                      db.query(jsonQuerySeeds, [fromDate, toDate], function(queryError, result) {
                        done();
                        if (queryError) {
                          res.sendStatus(500);
                        } else {
