...
router.get('/sold', function(req,res) {
  console.log('in planner getSold');
  var sold = [];
  if (req.isAuthenticated()) {
    pool.connect(function(err, connection, done) {
      if (err) {
