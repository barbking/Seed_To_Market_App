  ...
  if (req.isAuthenticated()) {
    pool.connect(function(err, connection, done) {
      if (err) {
        ...
        console.log('connected to db');
        var resultSet = connection.query("SELECT seeds.seed_id, seeds.crop, seeds.variety, planted.location, planted.planted_id FROM seeds INNER JOIN planted ON seeds.seed_id=planted.seed_id WHERE seeds.user_id=$1;", [req.user.user_id]);
        resultSet.on( 'row', function( row ){
